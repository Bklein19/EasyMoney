...
export function getDataCompleteness(input: { accountIds?: number[]; startDate?: string | null; endDate?: string | null; today?: string } = {}) {
  ...
  const ranges = getDb().prepare(`SELECT sa.accountId, sf.coveredFrom, sf.coveredTo, sf.coverageBasis,
    ...
    LEFT JOIN sourceTransactions st ON st.sourceAccountId = sa.id AND st.sourceFileId = sf.id
    WHERE sf.status = 'committed' AND sa.accountId IS NOT NULL
    GROUP BY sa.id`).all() as Array<{ accountId: number; coveredFrom: string | null; coveredTo: string | null; coverageBasis: string | null;
  ...
  const result = accounts.map(account => {
    ...
    const start = input.startDate?.slice(0, 10) || observedStart || end;
    ...
    return { accountId: account.accountId, accountName: account.accountName, closed: account.accountStatus === 'closed',
      start, end, latestTransactionDate: account.latestTransactionDate, latestBalanceDate: latestBalance.date,
      balanceStatus, transactionCoverage: gaps.length ? 'unverified' : 'declared', gaps,
