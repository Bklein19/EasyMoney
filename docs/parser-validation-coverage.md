...
Main integration acceptance (2026-09-15): 754 tests passed, standalone TypeScript checking, lint and client build passed, and all 954 retained statements passed strict replay. The merged retirement parsers preserve both `statementCashFlow` reporting evidence and `statementValidation` evidence, with a regression test for their coexistence. Normal desktop preparation/build still stalls in the runtime; this integration does not claim packaged-app acceptance or apply a live parser refresh.
